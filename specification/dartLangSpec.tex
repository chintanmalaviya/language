...
\LMHash{}%
A static extension application does not have a type.

\commentary{%
This is consistent with the fact that any use of a static extension application
...
\item
  ...
  \commentary{%
    ...
    Note that $r$ also does not denote a prefix or a static extension,
    and it is not a static extension application,
...
\item
  ...
  \commentary{%
    ...
    (\ref{typeVoid}),
    so static extensions are never applicable to receivers of
    any of the types \DYNAMIC, \code{Never}, or \VOID.%
   }

  For the purpose of determining static extension applicability,
  function types and the type \code{Function}
  are considered to have a member named \CALL.

  \commentary{%
    Hence, static extensions are never applicable to functions
    when the basename of the member is \CALL.
    %
    Instance members declared by the built-in class \code{Object}
    exist on all types,
    so no static extension is ever applicable for members with such names.%
  }
\item
  Consider a static extension application $a$ of the form \code{$E$($v$)}
  where $v$ is a fresh variable with static type $S$.
  It is required that an occurrence of $a$ at the location of $e$
...
\item
  The static extension $E$ declares an instance member with basename $m$.
\end{itemize}

\commentary{%
With type inference:
The context type of the invocation does not affect
whether the static extension is applicable,
and neither the context type nor the method invocation affects
the type inference of $r$,
but if the static extension method itself is generic,
...
\LMHash{}%
When \List{E}{1}{k}, $k > 1$, are static extensions
which are is accessible and applicable to a member invocation $e$
(\ref{memberInvocations}),
we define the notion of
\IndexCustom{specificity}{static extension!specificity},
which is a partial order on \List{E}{1}{k}.

\commentary{%
The specificity is used to determine
which static extension method to execute
in this situation where more than one choice is possible
(\ref{implicitStaticExtensionInvocations}).%
}
...
accessible and applicable static extensions for $e$.
...
Then $E_1$ is more specific than $E_2$ with respect $e$
...
\LMLabel{staticExtensionExamples}
...
\commentary{%
Here both the static extensions apply,
but the \code{SmartList} static extension is
