...
\commentary{%
The following examples illustrate implicit static extension resolution
when multiple applicable static extensions are available.%
}
...
\commentary{%
Here both of the static extensions apply,
...
so \code{BestSpec} is most selected.
Hence, the type of \code{y.best()} is \code{num}.%
}

\rationale{%
In general, the definition of specificity aims to select
the static extension which has more precise type information available.
...
In practice, we expect unintended static extension
member name conflicts to be rare.
If the same author is providing more specialized versions of
a static extension for subtypes,
the choice of a static extension which has the most precise types available
is likely to be a rather unsurprising and useful behavior.%
}


\subsection{Static analysis of Members of a Static Extension}
\LMLabel{staticAnalysisOfMembersOfAStaticExtension}

\LMHash{}%
Static analysis of the member declarations in a static extension $E$
relies on the scopes of the static extension
...
of a static extension $E$ with \ON{} type $T_{on}$,
...
\commentary{%
A lexical lookup in a static extension $E$ may yield
a declaration of an instance method declared in $E$.
As specified elsewhere
(\ref{lexicalLookup}),
this implies that static extension instance methods
will shadow class instance methods
when called from another extension instance method
inside the same extension just using the name
(that is, using \code{m()} and not \code{\THIS.m()}).
%
This is an exception,
because invocation of a static extension instance member with basename \id{}
will otherwise never occur when
the interface of the static type of the receiver has
...
and unless there are any other static extensions in scope creating a conflict.

The unqualified \code{length} in the declaration of \code{isEven}
...
even if there are other static extensions in scope
...
or it may be resolved using a different static extension.%
}
...
\item
  ...
  (\ref{ordinaryInvocation}).
\item
  If $D$ is an instance member of a static extension $E$
...
\LMHash{}%
If $e$ is a type literal or denotes a static extension, $i$ is treated as
...
\item
  ...
  a class $C$ and the interface of $C$ does not have a member named $n$;
  and it is a compile-time error if $\ell$ occurs in a static extension $E$,
...
%   the enclosing class / of the on-type of the enclosing static extension has
%   a member named $n$, or
...
\item
  Consider the case where $D$ is an instance member declaration in a class $C$.
...
\item
  ...
  % - a static method/getter/setter of a class, mixin, or static extension;
  % - an instance method of a static extension;
