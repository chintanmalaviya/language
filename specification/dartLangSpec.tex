...
% - Integrate extension methods into dartLangSpec.tex.
%
% 2.6
...
they are declared outside the target class, in an extension,
and they are resolved statically,
based on whether the relevant extension is in scope,
...
\LMHash{}%
A declaration derived from \synt{staticExtensionDeclaration} is known as an
...
\LMHash{}%
The current scope for an extension declaration is
the library scope of the enclosing library.
%
An extension declaration may be generic
(\ref{generics}),
which introduces the extension's
...
%
The current scope for the member declarations is the extension's
...
an extension instance member with basename \id{}
...
\item
  ...
  and it is a compile-time error if $\ell$ occurs in an extension $E$,
...
\item
  ...
  If $\ell$ occurs in an extension $E$ with \ON{} type $T_{on}$,
  the lexical lookup yields the member signature of
