...
Additionally,
the static extension is introduced into the library namespace
with a fresh, private name.
The former is known as the
\IndexCustom{declared name}{static extension!declared name}
of the static extension,
and the latter is known as the
\IndexCustom{fresh name}{static extension!fresh name}
of the static extension.

%% TODO(eernst): When https://github.com/dart-lang/language/pull/534 lands,
%% this paragraph should perhaps be moved into \ref{imports}.
\LMHash{}%
When the current library $L$ imports a library $L'$ such that
the imported namespace from $L'$ contains a name $n$ denoting
a static extension $E$,
a fresh private name $n_f$ is added to
the library scope of the current library,
bound to $E$.
\commentary{%
This implies that $n$ is a public name exported by $L'$
which is not removed using a \HIDE{} or \SHOW{} clause on the given import.%
}
The name $n_f$ is known as the
\NoIndex{fresh name}
of $E$ in the current library.

\commentary{%
This means that the declared name of $E$ is
in the library scope of the current library
following the same rules as the names of
other imported public declarations like classes.
%
When the declared name of $E$ is imported,
the fresh name of $E$ in the current library is also created.
%
However, the fresh name of $E$ is created in one \emph{additional} case,
namely when there is a name clash with some other imported library
on the declared name of $E$,
in which case it is an error to use the declared name.%
}

\commentary{%
The fresh name makes it possible for a static extension to be used
even in the case where the declared name is omitted,
or the declared name is shadowed by a declaration in an intermediate scope,
namely in an implicit invocation
(\ref{implicitStaticExtensionInvocations}).%
}

\rationale{%
The fact that a static extension can be used implicitly even in the case where
...
of a given static extension, an implicit invocation can use it.%
}

\LMHash{}%
The \synt{type} in a static extension declaration
is known as the static extension's
...
\LMHash{}%
The declared name of a static extension does not denote a type,
...
A compile-time error occurs if the declared name of a static extension
is a built-in identifier.

\LMHash{}%
The current scope for a static extension declaration is
the library scope of the enclosing library.
%
A static extension declaration may be generic
(\ref{generics}),
which introduces the static extension's
\IndexCustom{type parameter scope}{static extension!type parameter scope},
which has the library scope as its enclosing scope.
The type parameter scope is the current scope for
the type parameters and for the static extension's \ON{} \synt{type},
if it exists,
otherwise the library scope is their current scope.
%
The current scope for the member declarations is the static extension's
\IndexCustom{body scope}{static extension!body scope},
which has the type parameter scope as its enclosing scope if it exists,
and otherwise the library scope.

\LMHash{}%
Let $D$ be a static extension declaration with declared name \code{E}.
A member declaration in $D$ with the modifier \STATIC{} is designated as a
\IndexCustom{static member}{static extension!static member}
declaration.
%
A member declaration in $D$ without the modifier \STATIC{} is designated as an
\IndexCustom{instance member}{static extension!instance member}
declaration.
%
...
\subsection{Explicit Invocation of an Instance Member of a Static Extension}
\LMLabel{explicitStaticExtensionInvocations}

\LMHash{}%
Let $E$ be a simple or qualified identifier that denotes a static extension.
A \IndexCustom{static extension application}{static extension!application}
is then an expression of the form
\syntax{$E$ <typeArguments>? `(' <expression> `)'}.
A static extension member with a name accessible to the current library
can be invoked explicitly on a particular object
by performing a member invocation
(\ref{memberInvocations})
where the receiver is a static extension application.
...
a static extension named \code{E} and declared as follows:%
}
...
\noindent
\commentary{%
Type inference for a static extension application
...
\LMHash{}%
Let $a$ be a static extension application of the form
\code{$E$<\List{T}{1}{s}>($e$)},
where $E$ denotes a static extension declared as
...
\LMHash{}%
We define the
\IndexCustom{instantiated \ON{} type}{static extension!instantiated \ON{} type}
of $a$ as $[T_1/X_1, \ldots, T_s/X_s]T$.
We define the
\IndexCustom{instantiation-to-bound \ON{} type}{
  static extension!instantiation-to-bound \ON{} type}
...
\LMHash{}%
It is a compile-time error if a static extension application occurs
in a location where it is \emph{not} the syntactic receiver of
a simple or composite member invocation
(\ref{memberInvocations}).

\commentary{%
That is, the only valid use of a static extension application is
to invoke members on it.
This is similar to how prefix names can also only be used as
member invocation targets,
except that static extensions can also declare operators.%
}

\LMHash{}%
Let $i$ be a simple member invocation
(\ref{memberInvocations})
whose receiver $r$ is a static extension application of the form
\code{$E$<\List{T}{1}{k}>($e$)}
...
A compile-time error occurs unless the static extension denoted by $E$
declares a member named $n$.
Otherwise let \List{X}{1}{k} be the type parameters of said static extension.
...
a variable whose declared type is a class that declares
a member $n$ with member signature
$[T_1/X_1, \ldots, T_k/X_k]s$
(\commentary{so when $E$ is non-generic, this is just $s$}).
Said member signature is called the
\IndexCustom{invocation member signature}{
  static extension!invocation member signature}
of $i$.
...
$e$ to an object $o$,
evaluating the actual arguments to an evaluated actual argument part
\code{<$u_1$, \ldots, $u_m$>($o_1, \ldots,\ o_n,\ x_{n+1}$: $o_{n+1}, \ldots,\ x_{n+k}$: $o_{n+k}$)},
incurring a dynamic error if an actual value argument does not have
a dynamic type which is a subtype of the corresponding actual parameter type,
...
whose receiver is a static extension application $a$ is a compile-time error.
This is so because it would force each \synt{cascadeSection}
to invoke a member of the same static extension,
which is unlikely to be desirable.%
}


\subsection{Implicit Invocation of an Instance Member of a Static Extension}
\LMLabel{implicitStaticExtensionInvocations}

\LMHash{}%
Instance members of a static extension can be invoked implicitly
(\commentary{without mentioning the name of the static extension}),
...
an explicit extension application with receiver $r$,
if a number of requirements that are detailed below are satisfied.%
}

\rationale{%
Implicit invocation is intended as the primary way to use static extensions,
...
a different static extension than the intended one.%
}

\LMHash{}%
An implicit static extension member invocation occurs
for a member invocation $i$
(\ref{memberInvocations})
with receiver $r$ and corresponding member name $m$ if{}f
(1) $i$ is a compile-time error
because the interface of the static type of $r$ does not have a member
whose basename is the basename of $m$,
and (2) there exists a unique most specific
(\ref{staticExtensionSpecificity})
static extension denoted by $E$ which is
accessible
(\ref{staticExtensionAccessibility})
and applicable
...
\LMHash{}%
Implicit invocation of an instance member of a static extension
...
perform an implicit static extension member invocation on \code{o} for
...
\subsubsection{Accessibility of a Static Extension}
\LMLabel{staticExtensionAccessibility}

\LMHash{}%
A static extension $E$ is
\IndexCustom{accessible}{static extension!accessibility}
in a given scope $S$
...
it is availably in \emph{any} scope,
...
\subsubsection{Applicability of a Static Extension}
\LMLabel{staticExtensionApplicability}

!!!TODO!!!

An static extension $E$ is *applicable* to a simple or composite member
invocation with corresponding member *basename* *m* and target
expression \code{e}, where \code{e} has static type *S*, if

- The invocation is an *instance* member invocation. That is the case if the expression \code{e} does not denote a prefix or a class, mixin or static extension declaration *(then the member invocation would be a static invocation)*, and it is not an explicit static extension application. An instance member invocation on \code{e} will always begin by evaluating \code{e} to an object, and then continue by performing an instance member invocation on that object.
- The type *S*  does not have a member with the basename  *m*. For this, the type \DYNAMIC{} is considered as having all member names, and an expression of type \code{Never} or \VOID{} cannot occur as the target of a member invocation, so none of these can ever have applicable static extensions. Function types and the type \code{Function} are considered as having a \CALL{} member. *This ensure that if there is an applicable static extension, the existing invocation would otherwise be a compile-time error*. Members of \code{Object} exists on all types, so they can never be the target of implicit member invocations (\commentary{they can also not be declared as static extension members}).
- The static extension application \code{E(x)} would be valid (not a compile-time error) where \code{x} is a fresh variable with static type $S$ (to avoid type inference for any type parameters of \code{E} from affecting the already determined static type of \code{e}) in a scope where \code{E} denotes the static extension.
...
the static extension applies, and neither the context type nor the method
invocation affects the type inference of \code{e}, but if the static extension
method itself is generic, the context type may affect the member
invocation.

\subsubsection{Specificity of a Static Extension}
\LMLabel{staticExtensionSpecificity}

When more than one static extension is accessible and applicable to a member
invocation, we define a partial ordering on those static extensions wrt. that
member invocation, so that we can choose the "best" candidate which
will have its static extension member be implicitly invoked.
...
different accessible and applicable static extensions for *i*. Let
...
1. The \code{E2} static extension is declared in a platform library and the
   \code{E1} static extension is not, or
...
The following examples display the implicit static extension resolution when
multiple applicable static extensions are available.
...
Here both the static extensions apply, but the \code{SmartList} static extension is
more specific than the \code{SmartIterable} static extension because
...
For \code{y.best()}, the most specific static extension is \code{BestSpec}.
...
In practice, unintended static extension method name conflicts are likely to
be rare. Intended conflicts happen where the same author is providing
more specialized versions of an static extension for subtypes, and in that
case, picking the static extension which has the most precise types available
...
Static member declarations in the static extension declaration can be
accessed the same way as static members of a class or mixin
declaration: By prefixing with the static extension's name.
...
denoting the static extension occurs in an expression except as the static extension
name of an static extension application or as the target of a (static) simple
or composite member invocation. In the latter case, it is a
**compile-time error** if the static extension does not declare a static
...
An static extension member invocation is a member invocation where the target
is an static extension application, or where the target is an object where we
perform implicit static extension application. At run-time, implicit
static extension invocations have been resolved and any type arguments will
have been inferred, so we can assume they are all known.

Evaluating the invocation performs a method invocation of the
corresponding instance member of the static extension, with `this` bound to
the receiver value and type parameters (both for the static extension and for
the member itself, if that is generic) bound to the types found by
static inference.

Prior to NNBD, all static extension members can be invoked on the null
object. Since \code{Null} is a subtype of the \ON{} type, this is
consistent behavior.

Post-NNBD, a non-nullable \ON{} type would not match a nullable
receiver type, so it is impossible to invoke an static extension method that
...
migrated static extension with a non-nullable \ON{} type is called on the
...
When executing an static extension instance member, we stated earlier that
...
Inside an static extension method body, \THIS{} does not refer to an instance
...
surrounding static extension.

Invocations on \THIS{} use the same static extension method resolution as any
other code. Most likely, the current static extension will be the only one in
...
static extension members, both static and instance, are in the *lexical*
scope of the static extension member body. That is why \code{MySmart} above
can invoke the static \code{smartHelper} without prefixing it by the
static extension name. In the same way, *instance* member declarations (the
static extension members) are in the lexical scope.

If an unqualified identifier inside an static extension instance member
lexically resolves to an static extension member of the surrounding static extension
(if the nearest enclosing declaration with the same basename is an
instance member of an static extension), then that identifier is not
equivalent to \code{\THIS.id}, rather the invocation is equivalent to
an explicit invocation of that static extension method on \THIS{} (which we
already know has a compatible type for the static extension):
\code{Ext<T1,…,Tn>(this).id}, where \code{Ext} is the surrounding
static extension and \code{T1} through \code{Tn} are its type parameters, if
any. The invocation works whether or not the names of the static extension or
parameters are actually accessible, it is not a syntactic rewrite.

If an unqualified identifier inside an static extension *static* member
lexically resolves to an static extension member, it is a **compile-time
...
static extensions in scope, it will call that. (Or unless someone adds an
...
\code{MyUnaryNumber(this).isEven}, even if there are other static extensions
...
members as usual. It is subject to static extension if \code{id} is not declared
...
An static extension can declare a member with the same (base-)name as a
...
to an existing class just because an static extension is already adding a
...
an override to keep using the static extension).

An unqualified identifier in the static extension can refer to any static extension
member declaration, so inside an static extension member body, \code{\THIS.add} and
\code{add} are not necessarily the same thing (if the \ON{} type has an
\code{add} member, then \code{\THIS.add} refers to that, while
\code{add} refers to the static extension method in the lexical scope). This
may be confusing. In practice, static extensions will rarely introduce
...
torn off function closes over both the static extension method, the receiver,
and any type arguments to the static extension, and if the tear-off is an
...
effectively creates a curried function from the static extension:
...
\commentary{%
  Torn off static extension methods are never equal unless they
  ...
  instantiated tear-off of a generic function), torn off static extension
  methods may close over the type variables of the static extension as
  well. To avoid distinction between generic and non-generic
  static extensions, no two torn off static extension methods are equal, even if
  they are torn off from the same static extension on the same object at the
  same static type.%
}

An static extension method torn off a *constant* receiver expression is not a
constant expression. It creates a new function object each time the
tear-off expression is evaluated.

An explicit static extension method application member invocation like
\code{Foo<Bar>(b).baz}, also creates a tear-off if \code{Foo.baz} is
an static extension method.
...
As the initial examples suggest, an static extension method named \CALL{} can
...
static extension \code{\OPERATOR []} would: \code{for (var i in 1[10])...}.
...
\code{call}). Otherwise we check for static extensions applying to the static type
of \code{e1} and declaring a \CALL{} member. If one such most specific
static extension exists, and it declares a \CALL{} static extension *method*, then
...
We could allow an applicable \CALL{} static extension method to be coerced
instead, as an implicit tear-off. We will not do so.

That is: We do *not* allow implicit tear-off of an static extension \CALL{}
...
while still attempting to make static extension methods act
...
methods will allow you to add "static extension methods", which are really
...
The static extension methods are *static*, which means that we use the static
type of an expression to figure out which method to call, and that
also means that static static extension methods are not virtual methods.
...
or invocation of static extension members.
  ...
  \begin{minipage}[h]{\textwidth}
    \begin{displaymath}
      \begin{array}{ll}
        ...
        \\
        \code{$r$[$e_1$] ??= $e_2$} &
        \code{\LET{} $v$\,=\,$e_1$, $v'$ = $r'$[$v$] \IN{} $v'$\,==\,\NULL\ %
          ?\ $r'$[$v$]\,=\,$e_2$\,:\,$v'$}
        \\
        \code{$r$[$e_1$] $\otimes$= $e_2$} &
        \code{\LET{} $v$\,=\,$e_1$ \IN{} $r'$[$v$] = $r'$[$v$] $\otimes$ $e_2$}
        \\
...
\LMHash{}%
When the receiver $r$ is a static extension application
(\ref{explicitStaticExtensionInvocations})
...
\LMHash{}%
When $r$ is not a static extension application,
$r'$ is a fresh variable bound to the value of $r$,
with the same static type as $r$.

\commentary{%
This corresponds to an extra outermost \LET{} in a number of rules
