...
(\ref{lexicalLookup}),
this implies that static extension instance members
will shadow class instance members
when called from another instance member inside the same static extension
using an unqualified function invocation
(that is, invoking it as \code{m()} and not \code{\THIS.m()},
\ref{unqualifiedInvocation}).
%
This is an exception,
because implicit invocation of
a static extension instance member with basename \id{}
...
\subsection{Static Extension Method Closurization}
\LMLabel{staticExtensionMethodClosurization}

\LMHash{}%
A static extension instance method is subject to closurization
in a similar manner as class instance methods
(\ref{superGetterAccessAndMethodClosurization}).

\LMHash{}%
Let $a$ be a static extension application
(\ref{explicitStaticExtensionInvocations})
of the form \code{$E$<\List{S}{1}{m}>($e_1$)}.
Let \List{Y}{1}{m} be the formal type parameters of the static extension $E$.
%
An expression $e$ of the form \code{$a$.\id}
where \id{} is an identifier is then known as a
\IndexCustom{static extension property extraction}{%
  static extension!property extraction}.
...
\IndexCustom{instance method closurization}{%
  static extension!instance method closurization}
of \id{} on $a$,
and evaluation of $e$ proceeds as follows:
...
\commentary{%
In other words, the closurization is
...
\commentary{%
Two static extension instance method closurizations are never equal
...
are instance method closurizations of the same static extension $E$
...
\commentary{%
Note that an instance method closurization on a static extension is not
...
!!!TODO!!! Say that generic function instantiation may apply.
...
a static method of a class, of a mixin, or of a static extension
(\ref{identifierReference});
